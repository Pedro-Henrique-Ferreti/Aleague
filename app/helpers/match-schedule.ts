...
export async function balanceScheduleWeeks(schedule: MatchSchedule, teamsCount: number, avoidGroupMembersCount?: number): Promise<MatchSchedule> {
  ...
  return isBalanced(rebalancedSchedule) ? rebalancedSchedule : schedule;
}
...
export async function createMatchSchedule(payload: MatchScheduleParams): Promise<MatchSchedule> {
  ...
  schedule = await balanceScheduleWeeks(schedule, teams.length, avoidGroups?.[0]?.length);
